...
pygame.display.set_caption("Sprite_bounce")
clock = pygame.time.Clock()

SPRITE_COLOR_EVENT = pygame.USEREVENT + 1
BG_COLOR_EVENT = pygame.USEREVENT + 2
SPRITE_COLORS = [pygame.Color(c) for c in ['yellow', 'blue', 'green', 'orange']]
BG_COLORS = [pygame.Color(c) for c in ['orange', 'pink', 'green', 'yellow']]
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
        elif event.type == SPRITE_COLOR_EVENT:
            sprite.change_color()
