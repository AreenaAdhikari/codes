...
def run_ai_teaching_assistant():
    ...
    tokens = st.slider("Max tokens (response lenght)", 200,2000,1000)
    memory = st.checkbox("Enable Conversation memory", value=True)
    col1,col2 = st.columns([1,2])
    if col1.button("Clear"):
        st.session_state.history_ata = []
        st.rerun()
    if "history_ata" in st.session_state and st.session_state.history_ata:
    ...
    if st.button("Ask"):
        ...
        for i,h in enumerate(st.session_state.history_ata, 1):
            ...
            if st.button(f"Regenerate Q{i}", key=f"regen_{i}"):
                new_ans = generate_complete(h["questions"], temp,tokens)
...
def run_math_mastermind():
    ...
    problem = st.text_area("Enter math difficulty level")
    level = st.selectbox("Dificulty",["Basic","Intermidiate", "Advanced"])
    if st.button("Solve"):
